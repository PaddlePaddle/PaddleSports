...
def roi_pool(input,
    ...
    Args:
        input (Tensor): Input feature, 4D-Tensor with the shape of [N,C,H,W], 
            ...
            The datasets type is float32 or float64.
        ...
        output_size (int or tuple[int, int]): The pooled output size(h, w), datasets type is int32. If int, h and w are both equal to output_size.
    ...
    ..  code-block:: python
        ...
        x = paddle.static.datasets(
                name='datasets', shape=[None, 256, 32, 32], dtype='float32')
        rois = paddle.static.datasets(
                name='rois', shape=[None, 4], dtype='float32')
        rois_num = paddle.static.datasets(name='rois_num', shape=[None], dtype='int32')
...
def roi_align(input,
    ...
    Args:
        ...
        rois (Tensor): ROIs (Regions of Interest) to pool over.It should be
            ...
            The datasets type is float32 or float64. Given as [[x1, y1, x2, y2], ...],
    ...
    Returns:
        ...
        Output: The output of ROIAlignOp is a 4-D tensor with shape (num_rois, channels, pooled_h, pooled_w). The datasets type is float32 or float64.
...
def iou_similarity(x, y, box_normalized=True, name=None):
    ...
    Args:
        ...
        y (Tensor): Box list Y holds M boxes, each box is represented as 
             ...
             bottom coordinate of the box. The datasets type is float32 or float64.
    ...
    Returns:
        Tensor: The output of iou_similarity op, a tensor with shape [N, M] 
              representing pairwise iou scores. The datasets type is same with x.
    ...
    Examples:
        .. code-block:: python
            ...
            x = paddle.static.datasets(name='x', shape=[None, 4], dtype='float32')
            y = paddle.static.datasets(name='y', shape=[None, 4], dtype='float32')
    ...
    **This OP only supports LoDTensor as input**. Concat multi-level RoIs 
    ...
    Args:
        multi_rois(list): List of RoIs to collect. Element in list is 2-D 
            LoDTensor with shape [N, 4] and datasets type is float32 or float64,
            N is the number of RoIs.
        multi_scores(list): List of scores of RoIs to collect. Element in list 
            is 2-D LoDTensor with shape [N, 1] and datasets type is float32 or
        ...
        rois_num_per_level(list, optional): The List of RoIs' numbers. 
            ...
            image on each level and the shape is [B] and datasets type is
    ...
    Returns:
        ...
        fpn_rois(Variable): 2-D LoDTensor with shape [N, 4] and datasets type is
        float32 or float64. Selected RoIs. 

        rois_num(Tensor): 1-D Tensor contains the RoIs's number of each 
        image. The shape is [B] and datasets type is int32. B is the number of
        images. 
    ...
    Examples:
        .. code-block:: python
            ...
            multi_scores = []
            for i in range(4):
                multi_rois.append(paddle.static.datasets(
                    name='roi_'+str(i), shape=[None, 4], dtype='float32', lod_level=1))
            for i in range(4):
                multi_scores.append(paddle.static.datasets(
...
def distribute_fpn_proposals(fpn_rois,
    ...
    **This op only takes LoDTensor as input.** In Feature Pyramid Networks 
    ...
    Args:
        ...
        fpn_rois(Variable): 2-D Tensor with shape [N, 4] and datasets type is
        ...
        rois_num(Tensor): 1-D Tensor contains the number of RoIs in each image. 
            The shape is [B] and datasets type is int32. B is the number of images.
    ...
    Returns:
        ...
        and datasets type of float32 and float64. The length is
        max_level-min_level+1. The proposals in each FPN level.

        restore_ind(Variable): A 2-D Tensor with shape [N, 1], N is 
        the number of total rois. The datasets type is int32. It is
        used to restore the order of fpn_rois.

        rois_num_per_level(List): A list of 1-D Tensor and each Tensor is 
        the RoIs' number in each image on the corresponding level. The shape 
        is [B] and datasets type of int32. B is the number of images
    ...
    Examples:
        .. code-block:: python
            ...
            from ppdet.modeling import ops
            paddle.enable_static()
            fpn_rois = paddle.static.datasets(
                name='datasets', shape=[None, 4], dtype='float32', lod_level=1)
...
def yolo_box(
    ...
    This operator generates YOLO detection boxes from output of YOLOv3 network.
     ...
     category number of source datasets(such as 80 in coco datasets), so the
    ...
    Args:
        ...
        origin_shape (Tensor): The image size tensor of YoloBox operator, This is a 2-D tensor with shape of [N, 2].
                    ...
                    scale. The datasets type is int32.
    ...
    .. code-block:: python
        ...
        paddle.enable_static()
        x = paddle.static.datasets(name='x', shape=[None, 255, 13, 13], dtype='float32')
        img_size = paddle.static.datasets(name='img_size',shape=[None, 2],dtype='int64')
...
def prior_box(input,
    ...
    Parameters:
       input(Tensor): 4-D tensor(NCHW), the datasets type should be float32 or float64.
       image(Tensor): 4-D tensor(NCHW), the input image datasets of PriorBoxOp,
            the datasets type should be float32 or float64.
    ...
    Examples:
        ...
        paddle.enable_static()
        input = paddle.static.datasets(name="input", shape=[None,3,6,9])
        image = paddle.static.datasets(name="image", shape=[None,3,9,12])
...
def multiclass_nms(bboxes,
    ...
    Examples:
        .. code-block:: python
            ...
            import paddle
            from ppdet.modeling import ops
            boxes = paddle.static.datasets(name='bboxes', shape=[81, 4],
                                      dtype='float32', lod_level=1)
            scores = paddle.static.datasets(name='scores', shape=[81],
                                      dtype='float32', lod_level=1)
    ...
    **Matrix NMS**
    ...
    Args:
        ...
        scores (Tensor): A 3-D Tensor with shape [N, C, M]
                           ...
                           are total M scores which corresponding M bounding
                           boxes. Please note, M is equal to the 2nd dimension
                           of BBoxes. The datasets type is float32 or float64.
        score_threshold (float): Threshold to filter out bounding boxes with
    ...
    Examples:
        .. code-block:: python
            import paddle
            from ppdet.modeling import ops
            boxes = paddle.static.datasets(name='bboxes', shape=[None,81, 4],
                                      dtype='float32', lod_level=1)
            scores = paddle.static.datasets(name='scores', shape=[None,81],
                                      dtype='float32', lod_level=1)
...
def bipartite_match(dist_matrix,
    ...
    Args:
        dist_matrix(Tensor): This input is a 2-D LoDTensor with shape
            [K, M]. The datasets type is float32 or float64. It is pair-wise
    ...
    Returns:
        ...
        matched_indices(Tensor): A 2-D Tensor with shape [N, M]. The datasets
        ...
        matched_distance(Tensor): A 2-D Tensor with shape [N, M]. The datasets
    ...
    **Box Coder Layer**
    ...
    Args:
        prior_box(Tensor): Box list prior_box is a 2-D Tensor with shape 
            [M, 4] holds M boxes and datasets type is float32 or float64. Each box
        ...
        prior_box_var(List|Tensor|None): prior_box_var supports three types 
            ...
            datasets type is float32 or float64. The second is list consist of
            4 elements shared by all boxes and datasets type is float32 or float64.
            Other is None and not involved in calculation. 
        target_box(Tensor): This input can be a 2-D LoDTensor with shape 
            [N, 4] when code_type is 'encode_center_size'. This input also can 
            be a 3-D Tensor with shape [N, M, 4] when code_type is 
            'decode_center_size'. Each box is represented as 
            [xmin, ymin, xmax, ymax]. The datasets type is float32 or float64.
    ...
    Examples:
        ...
        .. code-block:: python
            ...
            paddle.enable_static()
            # For encode
            prior_box_encode = paddle.static.datasets(name='prior_box_encode',
                                  shape=[512, 4],
                                  dtype='float32')
            target_box_encode = paddle.static.datasets(name='target_box_encode',
                                   shape=[81, 4],
                                   dtype='float32')
            output_encode = ops.box_coder(prior_box=prior_box_encode,
                                    ...
                                    code_type="encode_center_size")
            # For decode
            prior_box_decode = paddle.static.datasets(name='prior_box_decode',
                                  shape=[512, 4],
                                  dtype='float32')
            target_box_decode = paddle.static.datasets(name='target_box_decode',
                                   shape=[512, 81, 4],
                                   dtype='float32')
    ...
    **Generate proposal Faster-RCNN**
    ...
    Args:
        scores(Tensor): A 4-D Tensor with shape [N, A, H, W] represents
            ...
            width of the feature map. The datasets type must be float32.
        bbox_deltas(Tensor): A 4-D Tensor with shape [N, 4*A, H, W]
            represents the difference between predicted box location and
            anchor location. The datasets type must be float32.
        im_shape(Tensor): A 2-D Tensor with shape [N, 2] represents H, W, the
            origin image size or input size. The datasets type can be float32 or
            float64.
        anchors(Tensor):   A 4-D Tensor represents the anchors with a layout
            of [H, W, A, 4]. H and W are height and width of the feature map,
            num_anchors is the box count of each position. Each anchor is
            in (xmin, ymin, xmax, ymax) format an unnormalized. The datasets type must be float32.
        variances(Tensor): A 4-D Tensor. The expanded variances of anchors with a layout of
            [H, W, num_priors, 4]. Each variance is in
            (xcenter, ycenter, w, h) format. The datasets type must be float32.
        pre_nms_top_n(float): Number of total bboxes to be kept per
            image before NMS. The datasets type must be float32. `6000` by default.
        post_nms_top_n(float): Number of total bboxes to be kept per
            image after NMS. The datasets type must be float32. `1000` by default.
        nms_thresh(float): Threshold in NMS. The datasets type must be float32. `0.5` by default.
        min_size(float): Remove predicted boxes with either height or
            width < min_size. The datasets type must be float32. `0.1` by default.
    ...
    Returns:
        ...
        - **rpn_rois**: The generated RoIs. 2-D Tensor with shape ``[N, 4]`` while ``N`` is the number of RoIs. The datasets type is the same as ``scores``.
        - **rpn_roi_probs**: The scores of generated RoIs. 2-D Tensor with shape ``[N, 1]`` while ``N`` is the number of RoIs. The datasets type is the same as ``scores``.
    ...
    Examples:
        .. code-block:: python
            ...
            from ppdet.modeling import ops
            paddle.enable_static()
            scores = paddle.static.datasets(name='scores', shape=[None, 4, 5, 5], dtype='float32')
            bbox_deltas = paddle.static.datasets(name='bbox_deltas', shape=[None, 16, 5, 5], dtype='float32')
            im_shape = paddle.static.datasets(name='im_shape', shape=[None, 2], dtype='float32')
            anchors = paddle.static.datasets(name='anchors', shape=[None, 5, 4, 4], dtype='float32')
            variances = paddle.static.datasets(name='variances', shape=[None, 5, 10, 4], dtype='float32')
