...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    """
    inputs_flat = paddle.reshape(inputs, [-1, inputs.shape[-1]])
    ...
    if viewdirs is not None:
        ...
        embedded = paddle.concat([embedded, embedded_dirs], -1)

    outputs_flat = batchify(fn, netchunk)(embedded)
    outputs = paddle.reshape(outputs_flat, list(inputs.shape[:-1]) + [outputs_flat.shape[-1]])
    return outputs
...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    near, far = near * paddle.ones_like(rays_d[...,:1]), far * paddle.ones_like(rays_d[...,:1])
    ...
    for k in all_ret:
        ...
        all_ret[k] = paddle.reshape(all_ret[k], k_sh)
...
def create_nerf(args):
    ...
    if args.ft_path is not None and args.ft_path!='None':
        ...
        ckpts = [os.path.join(basedir, expname, f) for f in sorted(os.listdir(os.path.join(basedir, expname))) if 'pdparams' in f]
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        # Load model
        model.set_state_dict(ckpt['network_fn_state_dict'])
        if model_fine is not None:
            model_fine.set_state_dict(ckpt['network_fine_state_dict'])
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    weights = alpha * paddle.cumprod(paddle.concat([paddle.ones((alpha.shape[0], 1)), 1.-alpha + 1e-10], axis=-1), dim=-1)[:, :-1]
...
def render_rays(ray_batch,
    ...
    bounds = paddle.reshape(ray_batch[...,6:8], [-1,1,2])
    near, far = bounds[...,0], bounds[...,1] # [-1,1]

    t_vals = paddle.linspace(0., 1., num=N_samples)
    if not lindisp:
    ...
    if perturb > 0.:
        ...
        upper = paddle.concat([mids, z_vals[...,-1:]], axis=-1)
        lower = paddle.concat([z_vals[...,:1], mids], axis=-1)
        # stratified samples in those intervals
        t_rand = paddle.rand(z_vals.shape)

        # Pytest, overwrite u with numpy's fixed random numbers
        if pytest:
            np.random.seed(0)
            t_rand = np.random.rand(*list(z_vals.shape))
            t_rand = paddle.to_tensor(t_rand)
...
#         raw = run_network(pts, fn=run_fn)
    ...
    if N_importance > 0:
        ...
        ret['acc0'] = acc_map_0
        ret['z_std'] = paddle.std(z_samples, axis=-1, unbiased=False)  # [N_rays]

    for k in ret:
        if (paddle.isnan(ret[k]).any() or paddle.isinf(ret[k]).any()) and DEBUG:
...
def config_parser():
    ...
    parser.add_argument("--rdpose_path", type=str, default=None, 
                        help='render pose .txt file to be rendered')
...
def test():
    ...
    if args.dataset_type == 'llff':
        ...
        # save_dir = r"../render_poses"
        # for i in range(len(render_poses)):
        #     np.savetxt(os.path.join(save_dir, 'render_poses_{}.txt'.format(i)), render_poses[i],fmt='%.8f', delimiter=' ')

        txt_path = args.rdpose_path
        pose = np.loadtxt(txt_path)
        render_poses = np.expand_dims(pose, axis=0).astype(np.float32)
    ...
    render_poses = paddle.to_tensor(render_poses)
