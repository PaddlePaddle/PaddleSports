...
def parse_args():
    ...
    parser.add_argument(
        '--folder_path', help='The image folder path that to be predicted.', type=str)
    parser.add_argument(
...
def seg_image_folder(args):
    assert os.path.exists(args.folder_path), \
        "The --folder_path is not existed: {}.".format(args.folder_path)

    logger.info("Create predictor...")
    predictor = Predictor(args)
    logger.info("Start predicting...")

    filenames = [f for f in os.listdir(args.folder_path) if f.endswith('.png')]
    ...
    for filename in filenames:
        ...
        bg_img = get_bg_img(args.bg_img_path, img.shape)
        out_img = predictor.run(img, bg_img)
        cv2.imwrite(os.path.join(args.save_dir, filename), out_img)
