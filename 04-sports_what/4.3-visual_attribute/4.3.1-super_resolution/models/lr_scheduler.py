...
class MultiStepRestartLR(LRScheduler):
    ...
    def __init__(self, optimizer, milestones, gamma=0.1, restarts=(0, ), restart_weights=(1, ), last_epoch=-1):
        ...
        super(MultiStepRestartLR, self).__init__(last_epoch)

    def get_lr(self):
        if self.last_epoch in self.restarts:
            weight = self.restart_weights[self.restarts.index(self.last_epoch)]
            # return [group['initial_lr'] * weight for group in self.optimizer.param_groups]
            return [self.optimizer._learning_rate*weight]
        if self.last_epoch not in self.milestones:
            # return [group['lr'] for group in self.optimizer.param_groups]
            return [self.optimizer._learning_rate]
        # return [group['lr'] * self.gamma**self.milestones[self.last_epoch] for group in self.optimizer.param_groups]
        return [self.optimizer._learning_rate * self.gamma ** self.milestones[self.last_epoch]]
