...
class BaseModel():
    ...
    def model_ema(self, decay=0.999):
        ...
        for k in net_g_ema_params.keys():
            # net_g_ema_params[k].data.mul_(decay).add_(net_g_params[k].data * (1 - decay))
            net_g_ema_params[k]= net_g_ema_params[k]*decay+(net_g_params[k]*(1 - decay))
    ...
    def get_optimizer(self, optim_type, params, lr, **kwargs):
        if optim_type == 'Adam':
            optimizer = paddle.optimizer.Adam(lr, parameters=params, **kwargs)
        else:
    ...
    def _print_different_keys_loading(self, crt_net, load_net, strict=True):
        ...
        if not strict:
            ...
            for k in common_keys:
                if crt_net[k].shape != load_net[k].shape:
