...
class RealESRGANModel(SRGANModel):
    ...
    def _dequeue_and_enqueue(self):
        ...
        if not hasattr(self, 'queue_lr'):
            ...
            self.queue_lr = paddle.zeros((self.queue_size, c, h, w))
            _, c, h, w = self.gt.shape
            self.queue_gt = paddle.zeros((self.queue_size, c, h, w))
            self.queue_ptr = 0
    ...
    def feed_data(self, data):
        if self.is_train and self.opt.get('high_order_degradation', True):
            ...
            self.kernel1 = data['kernel1'].astype(paddle.float32)
            self.kernel2 = data['kernel2'].astype(paddle.float32)
            self.sinc_kernel = data['sinc_kernel']

            ori_h, ori_w = self.gt.shape[2:4]
            ...
            if mode == 'area':
                ...
                out = F.interpolate(out, scale_factor=[scale, scale], mode=mode)
            # noise
            ...
            if np.random.uniform() < self.opt['gaussian_noise_prob']:
                ...
                out = random_add_poisson_noise_pt(
                    ...
                    rounds=False)
            # JPEG compression
            jpeg_p = paddle.uniform([out.shape[0]], dtype='float32', min=self.opt['jpeg_range'][0],
                                    max=self.opt['jpeg_range'][1])
            ...
            if np.random.uniform() < 0.5:
                ...
                # JPEG compression
                jpeg_p = paddle.uniform([out.shape[0]], dtype='float32', min=self.opt['jpeg_range2'][0],
                                        max=self.opt['jpeg_range2'][1])
                # jpeg_p = out.new_zeros(out.shape[0]).uniform_(*self.opt['jpeg_range2'])
                out = paddle.clip(out, 0, 1)
                out = self.jpeger(out, quality=jpeg_p)
            else:
                # JPEG compression
