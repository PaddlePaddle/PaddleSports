...
def parse_opt():
    ...
    parser.add_argument('--use_bn', type=int, default=0,
                        help='If 1, then do batch_normalization first in att_embed, if 2 then do bn both in the beginning and the end of '
                             'att_embed')
    ...
    parser.add_argument('--seq_per_img', type=int, default=5,
                        help='number of captions to sample for each image during training. Done for efficiency since CNN forward pass is '
                             'expensive. E.g. coco has 5 sents/image')

    # Sample related
    parser.add_argument('--beam_size', type=int, default=1,
                        help='used when sample_method = greedy, indicates number of beams in beam search. Usually 2 or 3 works well. More '
                             'is not better. Set this to 1 for faster runtime but a bit worse performance.')
    ...
    parser.add_argument('--weight_decay', type=float, default=0.0,  # paddle里必须为float类型
                        help='weight_decay')
    ...
    parser.add_argument('--bleu_reward_weight', type=float, default=0.0,
                        help='The reward weight from bleu1')

    # config
    parser.add_argument('--cfg', type=str, default='configs/aoa_rl.yml',
...
def add_eval_options(parser):
    ...
    parser.add_argument('--language_eval', type=int, default=0,
                        help='Evaluate language as well (1 = yes, 0 = no)? BLEU/CIDEr/METEOR/ROUGE_L? requires coco-caption code from '
                             'Github.')
    ...
    parser.add_argument('--beam_size', type=int, default=2,
                        help='indicates number of beams in beam search. Usually 2 or 3 works well. More is not better. Set this to 1 for '
    ...
    parser.add_argument('--coco_json', type=str, default='',
                        help='if nonempty then use this file in DataLoaderRaw (see docs there). Used only in MSCOCO test evaluation, '
                             'where we have a specific json file of only test set images.')
    # misc
    parser.add_argument('--id', type=str, default='',
