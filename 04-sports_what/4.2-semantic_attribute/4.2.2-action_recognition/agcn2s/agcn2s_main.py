...
from agcn2s_framework import AGCN2S_framework
from agcn2s_dataset import Dataset
import os
...
class AGCN2S_main():
    ...
    def train(self):
        ...
        if self.load_pretrain_model:
            agcn2s.set_state_dict(paddle.load(self.load_pretrain_model))
        crossEntropyLoss =nn.CrossEntropyLoss()
        scheduler_G = CustomWarmupAdjustDecay(0.1,5,0.1,[ 30, 40 ],1)
        optimizer = paddle.optimizer.Momentum(learning_rate=scheduler_G, momentum=0.9, parameters=agcn2s.parameters(), use_nesterov=False, weight_decay=1e-4, grad_clip=None, name=None)
        ...
        for epoch in range(epoches):
            ...
            if epoch%2 == 0:
                agcn2s.eval()
                v_acc = self.valid_accurary(valid_loader,agcn2s)
                agcn2s.train()
                ...
                if v_acc > v_acc_max:
                    ...
                    paddle.save(agcn2s.state_dict(), save_param_path_model)
    ...
    def test(self):
        agcn2s = AGCN2S_framework()
        agcn2s.set_state_dict(paddle.load(self.load_pretrain_model))
        valid_acc = self.valid_accurary(self.data_loader,agcn2s)# 0.52
