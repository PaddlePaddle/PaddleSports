...
from ctrgcn_framework import CTRGCN_framework
from ctrgcn_dataset import Dataset
import os
...
class CTRGCN_main():
    ...
    def train(self):
        ...
        if self.load_pretrain_model:
            ctrgcn.set_state_dict(paddle.load(self.load_pretrain_model))
        ...
        for epoch in range(epoches):
            ...
            if epoch%2 == 0:
                ctrgcn.eval()
                v_acc = self.valid_accurary(valid_loader,ctrgcn)
                ctrgcn.train()
    ...
    def test(self):
        ctrgcn = CTRGCN_framework()
        ctrgcn.set_state_dict(paddle.load(self.load_pretrain_model))
        valid_acc = self.valid_accurary(self.data_loader,ctrgcn)# 0.52
