...
from data.source.widerface import WIDERFaceDataSet
...
class Trainer(object):
    def __init__(self, cfg, mode='train'):
        ...
        if self.mode == 'train':
            # use WIDERFaceDataSet
            self.dataset = WIDERFaceDataSet(**cfg['TrainDataset']['WIDERFaceDataSet'])
        ...
        # build model
        assert cfg['architecture'] in ['BlazeFace'],'architecture {} not available!'.format(cfg['architecture'])
        if cfg['architecture'] == 'BlazeFace':
            _architectures = getattr(architectures, 'BlazeFace', None)
        ...
        if self.mode == 'eval':
            # use WIDERFaceDataSet
            self.dataset = WIDERFaceDataSet(**cfg['EvalDataset']['WIDERFaceDataSet'])
    ...
    def train(self, validate=False):
        ...
        Init_mark = False
        if validate:
            self.eval_dataset = WIDERFaceDataSet(
                dataset_dir=self.cfg['EvalDataset']['WIDERFaceDataSet']['dataset_dir'],
                image_dir=self.cfg['EvalDataset']['WIDERFaceDataSet']['image_dir'],
                anno_path=self.cfg['EvalDataset']['WIDERFaceDataSet']['anno_path'],
            )
    ...
    def predict(self,
                ...
                output_dir='output',
                save_results=False):
        self.dataset = DetDataset(**self.cfg['TestDataset']['ImageFolder'])

        # self.dataset.set_images(images)

        loader_ = TestReader(**self.cfg['TestReader'])
        loader = loader_(
            dataset=self.dataset,
            worker_num=0,
            infer=True
