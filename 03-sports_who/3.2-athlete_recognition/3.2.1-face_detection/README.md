简体中文 | [English](https://github.com/PaddlePaddle/PaddleDetection/blob/release/2.5/configs/face_detection/README.md)
...
```
cd data/wider_face && ./download_wider_face.sh
```

### 参数配置
基础模型的配置可以参考`configs/_base_/blazeface.yml`；
改进模型增加FPN和SSH的neck结构，配置文件可以参考`configs/_base_/blazeface_fpn.yml`，可以根据需求配置FPN和SSH，具体如下：
```yaml
BlazeNet:
...
[comment]: <> (* 如果您想了解具体细节可以参考AIstudio-[-])
...
```shell
python  train.py -c configs/blazeface_1000e.yml --amp
```
...
- 步骤一：评估并生成结果文件：
```shell
python eval.py -c configs/blazeface_1000e.yml \
       -w output/BlazeFace/model_final \
       multi_scale=True
```
