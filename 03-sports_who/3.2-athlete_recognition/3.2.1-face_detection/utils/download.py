...
DATASET_HOME = osp.expanduser("~/.cache/paddle/datasets")
...
def get_dataset_path(path, annotation, image_dir):
    ...
    Otherwise, get datasets path from DATASET_HOME, if not exists,
    download it.
    """
        ...
        if data_name == name:
            ...
            if name == 'mot':
                if osp.exists(path) or osp.exists(data_dir):
                    ...
                    raise NotImplementedError(
                        "Dataset {} is not valid for download automatically. "
                        "Please apply and download the datasets following docs/tutorials/PrepareMOTDataSet.md".
                        format(name))
    ...
    # not match any datasets in DATASETS
    raise ValueError(
        "Dataset {} is not valid and cannot parse datasets type "
...
def create_voc_list(data_dir, devkit_subdir='VOCdevkit'):
    ...
    # datasets, VOC default label list should be used,
    # do not generate label_list.txt here. For default
    # label, see ../datasets/source/voc.py
    ...
    if image_dir:
        ...
        if not osp.isdir(image_path):
            logger.warning("Config image_dir {} is not a "
                           "directory, datasets config is not "
...
def _decompress_dist(fname):
    ...
    if 'PADDLE_TRAINERS_NUM' in env and 'PADDLE_TRAINER_ID' in env:
        ...
        if num_trainers <= 1:
            ...
            # time, e.g in tiny datasets such as coco_ce, spine_coco, main
