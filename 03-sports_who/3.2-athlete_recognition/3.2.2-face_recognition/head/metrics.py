...
class ArcFace(nn.Layer):
    ...
    def __init__(self, embedding_size, class_dim, margin=0.5, scale=64.0, easy_margin=False):
    ...
    def forward(self, input, label):
        # norm input
        # print(input.shape) #[1024,128]
        input_norm = paddle.sqrt(paddle.sum(paddle.square(input), axis=1, keepdim=True))
        # print(input_norm.shape)   #[1024,1]
        input = paddle.divide(input, input_norm)  # support broadcast
        # print(input.shape) #[1024,128]
        # norm weight
        # print(self.weight.shape) #[128,8631]
        w_square = paddle.square(self.weight)
        w_sum = paddle.sum(w_square, axis=0, keepdim=True)
        weight_norm = paddle.sqrt(w_sum)
        # print(weight_norm.shape) #[1,8631]
        weight = paddle.divide(self.weight, weight_norm)
        # print(weight.shape) #[128,8631]
        # get cos(sita)
        cos = paddle.matmul(input, weight)  # [batch_size,class_dim]
        ...
        if self.easy_margin:
            ...
            phi = self._paddle_where_more_than(cos, self.th, phi, cos - self.mm)
        # use label
...
class Am_softmax(nn.Layer):
    ...
    def __init__(self, in_features, out_features, m=0.35, s=30.0):
        ...
        # ###self.kernel.datasets.uniform_(-1, 1).renorm_(2, 1, 1e-5).mul_(1e5)  # initialize kernel
