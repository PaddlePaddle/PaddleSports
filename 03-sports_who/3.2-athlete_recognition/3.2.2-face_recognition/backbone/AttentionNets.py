...
class ResidualBlock(nn.Layer):
    def __init__(self, input_channels, output_channels, stride=1):
        ...
        self.relu = nn.ReLU()
        self.conv1 = nn.Conv2D(input_channels, output_channels // 4, 1, 1)
        self.bn2 = nn.BatchNorm2D(output_channels // 4)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2D(output_channels // 4, output_channels // 4, 3, stride, padding=1)
        self.bn3 = nn.BatchNorm2D(output_channels // 4)
        self.relu = nn.ReLU()
        self.conv3 = nn.Conv2D(output_channels // 4, output_channels, 1, 1)
        self.conv4 = nn.Conv2D(input_channels, output_channels, 1, stride)
...
class AttentionModule_stage1(nn.Layer):
    # input size is 56*56
    ...
    def forward(self, x):
        ...
        # print(out_skip2_connection.datasets)
        # print(out_interp3.datasets)
...
class AttentionModule_stage2(nn.Layer):
...
class AttentionModule_stage3(nn.Layer):
...
class ResidualAttentionNet(nn.Layer):
