...
class ResNet(nn.Layer):
    ...
    def __init__(self,
        ...
        Args:
            ...
            freeze_at (int): freeze the backbones at which stage
        ...
        if input_size[0] == 112:
            self.output_fc = paddle.nn.Linear(2048 * 4 * 4, embedding_size)
        else:
            self.output_fc = paddle.nn.Linear(2048 * 7 * 7, embedding_size)
        self.output_flatten = paddle.nn.Flatten(start_axis=1)
        self.bn_output = paddle.nn.BatchNorm(embedding_size)
        self.depth = depth
...
def load_weight(model, weight_path, downloaded=True, optimizer=None):
    ...
    model_weight = {}
    incorrect_keys = 0
    extended_layer = ['output_fc.weight', 'output_fc.bias', 'bn_output.weight', 'bn_output.bias', 'bn_output._mean',
                      'bn_output._variance']

    for key in model_dict.keys():
        if downloaded:
            transform_key = 'backbones.' + key
        else:
            transform_key = key
